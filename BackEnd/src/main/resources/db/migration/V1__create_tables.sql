...
create table itens (data timestamp(6), id bigint generated by default as identity, usuario_id bigint, descricao varchar(255), localizacao varchar(255), nome varchar(255), status varchar(255) check (status in ('MATCHING','EM_DEVOLUCAO','RECUPERADO')), tipo varchar(255) check (tipo in ('PERDIDO','ACHADO')), primary key (id));
create table match_item (item_id bigint not null, match_id bigint not null, primary key (item_id, match_id));
create table matchs (confirmacao_achado boolean not null, confirmacao_perdido boolean not null, id bigint generated by default as identity, primary key (id));
...
alter table if exists match_item add constraint FK5fmwi7kvrjo3tj0ffdx8tutxg foreign key (item_id) references itens;
alter table if exists match_item add constraint FK1cgw118945dnrlm2gdxps7vm6 foreign key (match_id) references matchs;
