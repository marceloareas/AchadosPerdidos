...
public class ItemMapper {
    ...
    public ItemMeusMatchesResponseDTO convertToItemMeusMatchesDTO(Item item){
        ItemMeusMatchesResponseDTO dto = new ItemMeusMatchesResponseDTO();
        dto.setId(item.getId());
        dto.setTipo(item.getTipo());
        dto.setNome(item.getNome());
        dto.setStatus(item.getStatus());
        ...
        List<CategoriaDTO> categorias = item.getCategorias().stream().map(categoriaMapper::convertToResponseDTO).toList();
        dto.setCategorias(categorias);
        dto.setPersonName(item.getUsuario().getNome());
