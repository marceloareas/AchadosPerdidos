...
public interface MatchRepository extends JpaRepository<Match, Long> {
    ...
    @Query("""
            ...
            WHERE m.isFinalizado = false
            AND (
               (m.itemPerdido.usuario.id = :userId AND m.arquivadoPorItemPerdido = false)
               OR
               (m.itemAchado.usuario.id = :userId AND m.arquivadoPorItemAchado = false)
            )
            """)
